...
SELECT 'Item Counts' AS section,
  (SELECT COUNT(*)
  FROM naaccr.NAACCR_ITEM)              AS naaccr_items,
  (SELECT COUNT(*)
  FROM naaccr.NAACCR_CONCEPT_MAP)       AS item_map_rows,
  (SELECT COUNT(*)
  FROM omop.concept
...
SELECT 'Value Counts' AS section,
  (SELECT COUNT(*)
  FROM naaccr.NAACCR_VALUE_CONCEPT_MAP) AS value_map_rows,
  (SELECT COUNT(*)
  FROM omop.concept
...
SELECT 'Synonym Counts' AS section,
  (SELECT COUNT(*)
  FROM omop.concept_synonym) AS total_synonyms,
  (SELECT COUNT(*)
...
FROM omop.concept_relationship hv
WHERE hv.relationship_id='Has value'
  AND NOT EXISTS (
    SELECT 1
  FROM omop.concept_relationship vo
...
FROM naaccr.NAACCR_VALUE_CONCEPT_MAP m
  JOIN omop.concept c ON c.concept_id = m.concept_id
