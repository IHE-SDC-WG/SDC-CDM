/*
  ...
  Notes:
    - Requires NAACCR metadata in naaccr.* (naaccr.NAACCR_ITEM, naaccr.SCHEMA_ITEM_CODE, naaccr.REGISTRY)
    - Requires prior execution of naaccr_omop_vocab.sql
    - Uses literal schema omop.* for OMOP tables
*/
...
FROM (VALUES
  ...
  LEFT JOIN omop.concept_class cc ON cc.concept_class_id = x.concept_class_id
...
SELECT 'Items: metadata vs map vs OMOP' AS section,
  (SELECT COUNT(*)
  FROM naaccr.NAACCR_ITEM) AS total_metadata_items,
  (SELECT COUNT(*)
  FROM naaccr.NAACCR_CONCEPT_MAP) AS mapped_items,
  (SELECT COUNT(*)
  FROM omop.concept c
    JOIN naaccr.NAACCR_CONCEPT_MAP m ON m.concept_id = c.concept_id) AS omop_item_concepts;
...
SELECT TOP 20
  ...
  i.item_num
FROM naaccr.NAACCR_ITEM i
  LEFT JOIN naaccr.NAACCR_CONCEPT_MAP m ON m.item_num = i.item_num
...
SELECT 'Values: metadata vs map vs OMOP' AS section,
  (SELECT COUNT(*)
  FROM naaccr.SCHEMA_ITEM_CODE) AS total_item_values,
  (SELECT COUNT(*)
  FROM naaccr.NAACCR_VALUE_CONCEPT_MAP) AS mapped_item_values,
  (SELECT COUNT(*)
  FROM omop.concept c
    JOIN naaccr.NAACCR_VALUE_CONCEPT_MAP v ON v.concept_id = c.concept_id) AS omop_value_concepts;
...
FROM naaccr.NAACCR_VALUE_CONCEPT_MAP v
  LEFT JOIN omop.concept c ON c.concept_id = v.concept_id
...
FROM naaccr.SCHEMA_ITEM_CODE sic
  LEFT JOIN naaccr.NAACCR_VALUE_CONCEPT_MAP v
...
SELECT 'Relationships: counts' AS section,
  ...
  FROM omop.concept_relationship cr
    JOIN omop.concept c1 ON c1.concept_id = cr.concept_id_1 AND c1.vocabulary_id = 'NAACCR2026'
    JOIN omop.concept c2 ON c2.concept_id = cr.concept_id_2 AND c2.vocabulary_id = 'NAACCR2026'
...
SELECT 'S2C maps: counts' AS section,
  (SELECT COUNT(*)
  FROM omop.source_to_concept_map s2c
...
SELECT TOP 20
  ...
  c.concept_class_id
FROM omop.source_to_concept_map s2c
  LEFT JOIN omop.concept c ON c.concept_id = s2c.target_concept_id
WHERE s2c.source_vocabulary_id = 'NAACCR2026'
...
SELECT TOP 20
  ...
  m.concept_id
FROM naaccr.NAACCR_CONCEPT_MAP m
  LEFT JOIN omop.concept_relationship cr ON cr.concept_id_1 = m.concept_id AND cr.relationship_id = 'Has value'
...
SELECT TOP 20
  ...
  v.concept_id
FROM naaccr.NAACCR_VALUE_CONCEPT_MAP v
  LEFT JOIN omop.concept_relationship cr ON cr.concept_id_1 = v.concept_id AND cr.relationship_id = 'Value of'
WHERE cr.concept_id_1 IS NULL
ORDER BY v.item_num, v.code;
