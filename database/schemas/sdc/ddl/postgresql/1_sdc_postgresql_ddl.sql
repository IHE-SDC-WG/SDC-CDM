...
CREATE TABLE IF NOT EXISTS sdc.template_term_map (
    template_term_map_id integer GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    template_sdc_id integer NOT NULL REFERENCES sdc.template_sdc(template_sdc_id),
...
CREATE TABLE IF NOT EXISTS sdc.template_map_content (
    template_map_content_id integer GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    template_term_map_id integer NOT NULL REFERENCES sdc.template_term_map(template_term_map_id),
    map_content text NULL
);

CREATE TABLE IF NOT EXISTS sdc.sdc_specimen (
    sdc_specimen_id integer GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    parent_specimen_id integer NULL REFERENCES sdc.sdc_specimen(sdc_specimen_id),
    visit_occurrence_id integer NULL,
...
CREATE TABLE IF NOT EXISTS sdc.observation_specimens (
    observation_specimens_id integer GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    sdc_form_answer_id integer NOT NULL REFERENCES sdc.sdc_form_answer(sdc_form_answer_id),
    sdc_specimen_id integer NOT NULL REFERENCES sdc.sdc_specimen(sdc_specimen_id)
);

CREATE INDEX IF NOT EXISTS idx_sdc_report_accession
    ON sdc.sdc_report (report_accession);
CREATE INDEX IF NOT EXISTS idx_sdc_form_answer_instance_question
