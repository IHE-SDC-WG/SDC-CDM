#!/usr/bin/env python3
"""Tests for workbook-to-JSON NAACCR/OMOP mapping conversion."""
...
from convert_naaccr_omop_maps import build_spec  # noqa: E402
...
def test_converter_merges_person_mappings_and_normalizes_storage_language() -> None:
    ...
    assert sum(
        1 for mapping in items if mapping.get("person_mapping_applied")
    ) == 142
    ...
    for mapping in items:
        if (
            mapping.get("person_mapping_applied")
            and mapping["storage"].startswith("OMOP ")
        ):
            assert mapping.get("proposed_extension_table") is None, mapping[
                "concept_code"
            ]


if __name__ == "__main__":
    test_converter_merges_person_mappings_and_normalizes_storage_language()
