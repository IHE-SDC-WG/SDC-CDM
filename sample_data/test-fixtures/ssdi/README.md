...
`csv/` contains all 12 deterministic files produced from those payloads. The automated test fetches
through an injected offline transport, asserts that NAACCR validation occurs first, checks one call
per unique schema and table endpoint, and compares every output byte-for-byte.
