...
from import_sdc_form import process_xml
from crud import (
...
def main(hl7_file, database_url):
    ...
    try:
        print_data_from_hl7(session, hl7_message)
        # root = convert_hl7_to_xml(hl7_message)
        # process_xml(root, session)
        # session.commit()
    except Exception:
        session.rollback()
        logger.error(
            "An error occurred while processing the HL7 message:", exc_info=True
        )
    finally:
        session.close()


def print_data_from_hl7(session, hl7_message, exit_on_error=True):
...
def convert_hl7_to_xml(hl7_message):
    """
    :param hl7_message: The HL7 message to convert to XML
    :return: The XML representation of the HL7 message (as an etree.Element)
    """
    pass
...
def parse_arguments():
    parser = argparse.ArgumentParser(
        description=(
            """Import a NAACCR Vol V message into a SDC CDM database.
            Internally, this script should convert a NAACCR Vol V message to an SDC form in XML and import it to the database using the SDC form import script.
            """
        ),
    )
    parser.add_argument("hl7_file", help="Path to the hl7 message file to process")
...
if __name__ == "__main__":
    ...
    main(args.hl7_file, args.database_url)
